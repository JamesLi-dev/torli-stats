...
struct DeckRootView: View {
    ...
    private var visible: [Note] {
        deck.showAll ? store.active : Array(store.active.prefix(NotySettings.fanLimit))
    }
    private var hiddenCount: Int { max(0, store.active.count - NotySettings.fanLimit) }
    ...
    private func pillTop(panelHeight h: CGFloat) -> CGFloat {
        ...
        return (1.0 - NotySettings.deckYRatio) * max(0, h - pillH)
    }

    private func fanTop(_ lay: DeckLayout, panelHeight h: CGFloat) -> CGFloat {
        let pillH = DeckGeom.pillHeight(noteCount: max(1, store.active.count))
        let availableH = max(1, h - pillH)
        let pillCenter = (1.0 - NotySettings.deckYRatio) * availableH + pillH / 2
...
struct FanColumn: View {
    ...
    private func scheduleHoverOpen(_ id: String) {
        ...
        hoverWork = work
        DispatchQueue.main.asyncAfter(deadline: .now() + NotySettings.openOnHoverDelay, execute: work)
    }
    ...
    private func scheduleHoverPreview(_ note: Note) {
        ...
        previewWork = work
        DispatchQueue.main.asyncAfter(deadline: .now() + NotySettings.tabPreviewDelay, execute: work)
    }
...
struct MoreTab: View {
    ...
    var body: some View {
        ...
        .buttonStyle(TabPressStyle())
        .help(NotyL10n.plural("notes.more", count))
    }
}
    ...
    var body: some View {
        Button(action: action) {
            ZStack(alignment: .top) {
                ...
                Text(NotyL10n.text("note.new_tab").uppercased())
                    .font(Ink.tabFont)
                    .tracking(Ink.tabTracking)
                    .foregroundStyle(.secondary)
                    .frame(width: max(20, strip - DeckGeom.labelInset),
...
struct PlusButton: View {
    ...
    var body: some View {
        ...
        .help(NotyL10n.text("help.new_note"))
    }
}

/// NotySettings, one step below the new-note button. The pill's context menu still
...
struct CogButton: View {
    ...
    var body: some View {
        ...
        .onHover { hovering = $0 }
        .animation(.easeOut(duration: 0.15), value: hovering)
        .help(NotyL10n.text("help.settings"))
    }
}
...
extension View {
    func noteContextMenu(_ note: Note) -> some View {
        contextMenu {
            Button(note.pinned ? NotyL10n.text("action.unpin") : NotyL10n.text("action.pin")) { NoteStore.shared.togglePin(id: note.id) }
            Button(NotyL10n.text("action.archive")) { NoteStore.shared.setArchived(id: note.id, true) }
            Button(NotyL10n.text("help.cycle_colour")) { NoteStore.shared.cycleColor(id: note.id) }
            Divider()
