...
final class DeckModel: ObservableObject {
    ...
    // Mirrored from NotySettings so SwiftUI re-renders when a preference flips.
    @Published var style: DeckStyle = NotySettings.deckStyle
    @Published var alwaysShown: Bool = NotySettings.deckAlwaysShown
    @Published var pillHidden: Bool = NotySettings.deckPillHidden
    /// DeckGeom reads the scale straight from NotySettings; this mirror exists purely
    /// so a change to it invalidates the views that measure against it.
    @Published var scale: Double = NotySettings.deckScale
    @Published var onLeftEdge: Bool = NotySettings.deckOnLeftEdge
    @Published var fontSize: Double = NotySettings.noteFontSize
    @Published var markdown: Bool = NotySettings.markdownStyling
    @Published var noteSize = NotySettings.noteSize
    @Published var openOnHover: Bool = NotySettings.openOnHover
    @Published var tabPreview: Bool = NotySettings.tabPreview
    ...
    @Published var openedHeight: CGFloat = NotySettings.noteSize.height
...
final class DeckController: NSObject {
    ...
    private var restingState: DeckState { NotySettings.deckAlwaysShown ? .fan : .rest }
    ...
    func layout(for state: DeckState) {
        ...
        let onRight = !NotySettings.deckOnLeftEdge
        ...
        case .rest:
            ...
            let w = max(DeckGeom.pillWidth + 2, CGFloat(NotySettings.edgeWidth))
            let availableH = max(1, vis.height - h)
            let y = vis.minY + availableH * NotySettings.deckYRatio
    ...
    private func startIdleWatch() {
        ...
        idleTimer = Timer.scheduledTimer(withTimeInterval: 0.12, repeats: true) { [weak self] _ in
            ...
            case .fan where idle > NotySettings.fanIdleTimeout:
                self.collapse()
            case .expanded where idle > NotySettings.noteIdleTimeout && !self.openNoteIsPinned:
                self.dismiss()
            default: break
            }
        }
    ...
    func beginPillDrag(with initialEvent: NSEvent) {
        ...
        var targetOnLeft = NotySettings.deckOnLeftEdge
        var targetYRatio = NotySettings.deckYRatio
        ...
        NotySettings.deckOnLeftEdge = targetOnLeft
        NotySettings.deckYRatio = targetYRatio
    ...
    private var hotZone: NSRect {
        ...
        return NotySettings.deckOnLeftEdge
    ...
    func expand(_ id: String) {
        ...
        model.openedHeight = NotySettings.noteSize.height
        model.openedTop = nil
        manager?.deckDidActivate(self)
    ...
    func detachExpandedNote(at pointer: NSPoint) {
        ...
        let size = NotySettings.floatingNoteSize
    ...
    /// the pointer is off the deck. Called after NotySettings writes it.
    ...
    func showContextMenu(at event: NSEvent) {
        let menu = NSMenu()
        menu.addItem(withTitle: NotyL10n.text("menu.new_note"), action: #selector(TorliAppDelegate.newNote), keyEquivalent: "")
        menu.addItem(withTitle: NotyL10n.text("menu.all_notes"), action: #selector(TorliAppDelegate.openAllNotes), keyEquivalent: "")
        menu.addItem(withTitle: NotyL10n.text("menu.archive"), action: #selector(TorliAppDelegate.openArchive), keyEquivalent: "")
        menu.addItem(.separator())

        let overFS = NSMenuItem(title: NotyL10n.text("menu.show_over_fullscreen"),
                                action: #selector(TorliAppDelegate.toggleOverFullScreen), keyEquivalent: "")
        overFS.state = NotySettings.showOverFullScreen ? .on : .off
        menu.addItem(overFS)

        let styleItem = NSMenuItem(title: NotyL10n.text("menu.deck_style"), action: nil, keyEquivalent: "")
        ...
        for s in DeckStyle.allCases {
            ...
            it.state = NotySettings.deckStyle == s ? .on : .off
            styleMenu.addItem(it)
        }
        styleItem.submenu = styleMenu
        menu.addItem(styleItem)

        let fontItem = NSMenuItem(title: NotyL10n.text("menu.note_font"), action: nil, keyEquivalent: "")
        let fontMenu = NSMenu()
        for f in Ink.faces {
        ...
        let textItem = NSMenuItem(title: NotyL10n.text("menu.text_size"), action: nil, keyEquivalent: "")
        let textMenu = NSMenu()
        for entry in NotySettings.fontSizes {
            let it = NSMenuItem(title: NotyL10n.text(entry.nameKey), action: #selector(TorliAppDelegate.setFontSize(_:)),
                                keyEquivalent: "")
            it.representedObject = entry.size
            it.state = abs(NotySettings.noteFontSize - entry.size) < 0.01 ? .on : .off
            textMenu.addItem(it)
        }
        textItem.submenu = textMenu
        menu.addItem(textItem)

        let sizeItem = NSMenuItem(title: NotyL10n.text("menu.deck_size"), action: nil, keyEquivalent: "")
        let sizeMenu = NSMenu()
        for entry in NotySettings.deckSizes {
            let it = NSMenuItem(title: NotyL10n.text(entry.nameKey), action: #selector(TorliAppDelegate.setDeckScale(_:)),
                                keyEquivalent: "")
            it.representedObject = entry.scale
            it.state = abs(NotySettings.deckScale - entry.scale) < 0.01 ? .on : .off
            sizeMenu.addItem(it)
        }
        sizeItem.submenu = sizeMenu
        menu.addItem(sizeItem)

        let keepOpen = NSMenuItem(title: NotyL10n.text("menu.keep_deck_open"),
                                  action: #selector(TorliAppDelegate.toggleDeckAlwaysShown), keyEquivalent: "")
        keepOpen.state = NotySettings.deckAlwaysShown ? .on : .off
        menu.addItem(keepOpen)

        let leftEdge = NSMenuItem(title: NotyL10n.text("menu.dock_left"),
                                  action: #selector(TorliAppDelegate.toggleDeckEdge), keyEquivalent: "")
        leftEdge.state = NotySettings.deckOnLeftEdge ? .on : .off
        menu.addItem(leftEdge)

        if NSScreen.screens.count > 1 {
            let displayItem = NSMenuItem(title: NotyL10n.text("menu.display"), action: nil, keyEquivalent: "")
            let displayMenu = NSMenu()

            let allItem = NSMenuItem(title: NotyL10n.text("display.all"), action: #selector(TorliAppDelegate.setDisplayTarget(_:)), keyEquivalent: "")
            allItem.representedObject = "all"
            allItem.state = NotySettings.displayTarget == "all" ? .on : .off
            displayMenu.addItem(allItem)

            let mainItem = NSMenuItem(title: NotyL10n.text("display.main"), action: #selector(TorliAppDelegate.setDisplayTarget(_:)), keyEquivalent: "")
            mainItem.representedObject = "main"
            mainItem.state = NotySettings.displayTarget == "main" ? .on : .off
            ...
            for screen in NSScreen.screens {
                ...
                let title = screen == NSScreen.main ? NotyL10n.format("display.named_main", name) : name
                ...
                it.state = NotySettings.displayTarget == "id:\(id)" ? .on : .off
        ...
        let exportItem = NSMenuItem(title: NotyL10n.text("menu.export"), action: nil, keyEquivalent: "")
        let exportMenu = NSMenu()
        exportMenu.addItem(withTitle: NotyL10n.text("export.markdown_per_note"),
                           action: #selector(TorliAppDelegate.exportMarkdown), keyEquivalent: "")
        exportMenu.addItem(withTitle: NotyL10n.text("export.plain_per_note"),
                           action: #selector(TorliAppDelegate.exportPlainText), keyEquivalent: "")
        exportMenu.addItem(withTitle: NotyL10n.text("export.single_document"),
                           action: #selector(TorliAppDelegate.exportSingleFile), keyEquivalent: "")
        exportMenu.addItem(withTitle: NotyL10n.text("export.sticky_archive"),
                           action: #selector(TorliAppDelegate.exportStickies), keyEquivalent: "")
        exportItem.submenu = exportMenu
        menu.addItem(exportItem)
        menu.addItem(withTitle: NotyL10n.text("menu.import"), action: #selector(TorliAppDelegate.importStickies), keyEquivalent: "")
        menu.addItem(.separator())
        menu.addItem(withTitle: NotyL10n.text("menu.settings"), action: #selector(TorliAppDelegate.openNoteSettings), keyEquivalent: "")
        menu.addItem(withTitle: NotyL10n.text("menu.quit"), action: #selector(TorliAppDelegate.quit), keyEquivalent: "")
...
final class DeckManager {
    ...
    private func targetDisplayIDs() -> Set<CGDirectDisplayID> {
        ...
        let target = NotySettings.displayTarget
        if target == "all" {
