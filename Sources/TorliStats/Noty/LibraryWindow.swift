...
enum LibraryMode: String, CaseIterable, Identifiable {
    ...
    var title: String {
        switch self {
        case .all: NotyL10n.text("library.all_notes")
        case .archive: NotyL10n.text("library.archive")
        }
    }
}
...
final class LibraryWindow: NSObject, NSWindowDelegate {
    ...
    func windowWillClose(_ notification: Notification) {
        ...
        // unless NotySettings is still up.
...
struct LibraryView: View {
    ...
    private var sidebar: some View {
        VStack(spacing: 0) {
            ...
            HStack(spacing: 6) {
                ...
                TextField(NotyL10n.text("library.search_placeholder"), text: $model.query)
            ...
            if filtered.isEmpty {
                VStack(spacing: 6) {
                    ...
                    Text(model.query.isEmpty
                         ? (model.mode == .all
                            ? NotyL10n.text("library.no_notes")
                            : NotyL10n.text("library.no_archive"))
                         : NotyL10n.text("library.no_matches"))
            ...
            HStack(spacing: 12) {
                Button { NoteStore.shared.create() } label: {
                    Label(NotyL10n.text("menu.new_note"), systemImage: "plus").font(.system(size: 11.5))
                }
                .buttonStyle(.plain).foregroundStyle(.secondary)

                Menu {
                    Button(NotyL10n.text("export.markdown_per_note")) {
                        Transfer.export(.markdown, notes: NoteStore.shared.notes)
                    }
                    Button(NotyL10n.text("export.plain_per_note")) {
                        Transfer.export(.plainText, notes: NoteStore.shared.notes)
                    }
                    Button(NotyL10n.text("export.single_document")) {
                        Transfer.export(.singleFile, notes: NoteStore.shared.notes)
                    }
                    Button(NotyL10n.text("export.sticky_archive")) {
                        Transfer.export(.stickies, notes: NoteStore.shared.notes)
                    }
                    Divider()
                    Button(NotyL10n.text("menu.import")) { Transfer.importFiles() }
                } label: {
                    Label(NotyL10n.text("menu.export"), systemImage: "square.and.arrow.up").font(.system(size: 11.5))
                }
    ...
    private func row(_ note: Note) -> some View {
        ...
        .contextMenu {
            if note.archived {
                Button(NotyL10n.text("action.restore")) { NoteStore.shared.setArchived(id: note.id, false) }
            } else {
                Button(NotyL10n.text("action.archive")) { NoteStore.shared.setArchived(id: note.id, true) }
            }
            Divider()
            Button(NotyL10n.text("action.delete")) { NoteStore.shared.delete(id: note.id) }
        }
    }
    ...
    private var detail: some View {
        if let note = selected {
            ...
            VStack(spacing: 8) {
                ...
                Text(NotyL10n.text("library.select_note")).font(.system(size: 13)).foregroundStyle(.secondary)
            }
            .frame(maxWidth: .infinity, maxHeight: .infinity)
    ...
    var body: some View {
        VStack(spacing: 0) {
            HStack(spacing: 10) {
                ...
                .buttonStyle(.plain)
                .help(NotyL10n.text("help.pick_colour"))
                .contextMenu {
                ...
                ZStack(alignment: .leading) {
                    if title.isEmpty {
                        Text(note.hasCustomTitle ? NotyL10n.text("note.title_prompt") : (Note.derivedTitle(from: text).isEmpty ? NotyL10n.text("note.untitled") : Note.derivedTitle(from: text)))
                ...
                .contextMenu {
                    if note.hasCustomTitle {
                        Button(NotyL10n.text("note.title_reset")) {
                            title = ""
                ...
                Spacer()
                Text(NotyL10n.format("note.edited", Fmt.ago(note.modified)))
            ...
            NoteTextView(text: $text, ink: NSColor(pal.ink), bridge: bridge,
                         autofocus: false, fontSize: NotySettings.noteFontSize,
                         markdownEnabled: NotySettings.markdownStyling,
                         textDirection: note.textDirection,
                         styleToken: "\(note.color)|\(NotySettings.noteFontSize)|\(NotySettings.noteFontName)|\(NotySettings.markdownStyling)")
