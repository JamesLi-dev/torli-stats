...
struct NoteTextView: NSViewRepresentable {
    ...
    var markdownEnabled: Bool = NotySettings.markdownStyling
    ...
    var body: some View {
        Group {
            if let symbol = direction.symbol {
                ...
                Text(NotyL10n.text("direction.auto_short"))
...
struct NoteTextDirectionMenu: View {
    ...
    var body: some View {
        ...
        .tint(foreground)
        .fixedSize()
        .help(NotyL10n.format("help.text_direction", direction.title))
    }
}
...
struct NoteEditorView: View {
    ...
    private var sheet: some View {
        VStack(spacing: 0) {
            ...
            NoteTextView(text: $text, ink: NSColor(pal.ink),
                         ...
                         styleToken: "\(note.color)|\(deck.fontSize)|\(NotySettings.noteFontName)|\(deck.markdown)")
            footer
        }
    }
    ...
    private var header: some View {
        HStack(spacing: 8) {
            ...
            Spacer(minLength: 6)
            Text(savedAt.map { NotyL10n.format("note.saved", Fmt.ago($0)) }
                 ?? NotyL10n.text("note.not_saved"))
                .font(.system(size: 10))
            ...
            .help(note.pinned ? NotyL10n.text("help.unpin") : NotyL10n.text("help.pin"))
    ...
    private var findBar: some View {
        HStack(spacing: 6) {
            ...
            TextField(NotyL10n.text("note.find_placeholder"), text: Binding(
                get: { deck.findQuery ?? "" },
    ...
    private var footer: some View {
        HStack(spacing: 7) {
            ...
            Spacer(minLength: 8)
            footerButton(NotyL10n.text("action.archive")) {
                NoteStore.shared.setArchived(id: note.id, true)
                controller.collapse()
            }
            footerButton(NotyL10n.text("action.delete")) {
                NoteStore.shared.delete(id: note.id)
                controller.collapse()
            }
            footerButton(NotyL10n.text("action.close")) { controller.collapse() }
        }
        .padding(.horizontal, 14)
        .frame(height: 34)
    }
