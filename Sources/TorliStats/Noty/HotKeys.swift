...
final class HotKeys {
    ...
    func register(newNote: @escaping () -> Void,
        ...
        handlers = [
            (1, { NotySettings.scNewNote }, newNote),
            (2, { NotySettings.scAllNotes }, allNotes),
            (3, { NotySettings.scArchive }, archive),
            (4, { NotySettings.scCapture }, capture),
        ]
        reload()
    }

    /// Re-reads the stored shortcuts and rebinds. Called after a change in
    /// NotySettings, so a new combo takes effect without relaunching.
    func reload() {
        unregisterHotKeys()
        installHandler()
        for h in handlers {
