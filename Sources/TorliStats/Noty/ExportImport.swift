...
enum Transfer {
    ...
    static func export(_ format: Format, notes: [Note]) {
        guard !notes.isEmpty else {
            alert(NotyL10n.text("export.empty_title"), NotyL10n.text("export.empty_body"))
            return
        }
        NSApp.activate()
        switch format {
    ...
    private static func exportPerFile(_ notes: [Note], ext: String, render: (Note) -> String) {
        ...
        panel.prompt = NotyL10n.text("export.here")
        panel.message = NotyL10n.plural("export.choose_folder", notes.count, ext.uppercased())
        ...
        if written < notes.count {
            alert(NotyL10n.text("export.incomplete_title"),
                  NotyL10n.format("export.incomplete_body", written,
                              NotyL10n.plural("notes.count", notes.count)))
        }
    }

    private static func exportSingle(_ notes: [Note]) {
        let panel = NSSavePanel()
        panel.nameFieldStringValue = NotyL10n.format(
        ...
        let doc = notes.map { n -> String in
            let archiveSuffix = n.archived ? NotyL10n.text("export.metadata_archived_suffix") : ""
            let metadata = NotyL10n.format(
        ...
        let header = "# \(NotyL10n.text("export.document_title"))\n\n"
            + "\(NotyL10n.plural("notes.count", notes.count)) · \(Fmt.stamp.string(from: Date()))\n\n---\n\n"
    ...
    private static func exportArchive(_ notes: [Note]) {
        ...
        do {
            ...
            alert(NotyL10n.text("export.failed_title"), error.localizedDescription)
        }
    }
    ...
    static func importFiles() {
        ...
        panel.allowsOtherFileTypes = true
        panel.message = NotyL10n.text("import.choose_files")
        ...
        if failed.isEmpty {
            alert(NotyL10n.text("import.complete_title"), NotyL10n.plural("import.added", added))
        } else {
            alert(NotyL10n.text("import.problems_title"),
                  NotyL10n.format("import.problems_body", NotyL10n.plural("import.added", added),
    ...
    private static func safeName(_ n: Note) -> String {
        ...
        return trimmed.isEmpty
            ? NotyL10n.format("export.default_note_name", String(n.id.prefix(8)))
            : trimmed
    }
