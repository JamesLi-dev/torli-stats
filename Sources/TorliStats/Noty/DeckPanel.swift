...
enum DeckStyle: String, CaseIterable {
    ...
    var title: String {
        switch self {
        case .tabs: return NotyL10n.text("deck_style.labelled_tabs")
        case .compact: return NotyL10n.text("deck_style.colour_chips")
        }
    }
}
...
enum DeckGeom {
    ...
    static var scale: CGFloat { CGFloat(NotySettings.deckScale) }
    ...
    static var editorWidth: CGFloat { NotySettings.noteSize.width }
    static var editorHeight: CGFloat { NotySettings.noteSize.height }
...
final class DeckPanel: NSPanel {
    ...
    func applyLevel() {
        level = NotySettings.showOverFullScreen ? .statusBar : .floating
        ...
        if NotySettings.showOverFullScreen { behavior.insert(.fullScreenAuxiliary) }
