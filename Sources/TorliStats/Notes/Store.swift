...
final class Store {
    ...
    init(dbURL: URL = Paths.db) {
        if sqlite3_open_v2(dbURL.path, &db,
                           ...
                           nil) != SQLITE_OK {
            NSLog("Torli Notes: cannot open db at \(dbURL.path)")
        }
    ...
    private func exec(_ sql: String) {
        ...
        if sqlite3_exec(db, sql, nil, nil, &err) != SQLITE_OK, let err {
            NSLog("Torli Notes sql: \(String(cString: err))")
            sqlite3_free(err)
        }
    }
    ...
    func upsert(_ n: Note) {
        ...
        if sqlite3_step(st) != SQLITE_DONE {
            NSLog("Torli Notes: upsert failed — \(String(cString: sqlite3_errmsg(db)))")
        }
    }
