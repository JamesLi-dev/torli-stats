...
final class FloatingNote: NSObject, NSWindowDelegate {
    ...
    func windowDidEndLiveResize(_ notification: Notification) {
        ...
        NotesSettings.floatingNoteSize = panel.frame.size
    }
    ...
    /// makeKey only fronts it within Torli Notes — orderFrontRegardless is what lifts
    /// it above the app you are actually in.
    func focus() {
        lastActivity = Date()
        guard let panel else {
            NSLog("Torli Notes: focus — no floating panel")
            return
        }
        NSLog("Torli Notes: focus lift  visible=\(panel.isVisible) level=\(panel.level.rawValue)")
    ...
    private func startIdleWatch() {
        ...
        idleTimer = Timer.scheduledTimer(withTimeInterval: 2, repeats: true) { [weak self] _ in
            ...
            if Date().timeIntervalSince(self.lastActivity) > NotesSettings.noteIdleTimeout {
                self.tuck()
            }
        }
    }
...
private struct FloatingNoteView: View {
    ...
    private func header(_ note: Note, _ pal: NoteColor) -> some View {
        HStack(spacing: 8) {
            ...
            .help(NotesL10n.text("action.close"))
        }
