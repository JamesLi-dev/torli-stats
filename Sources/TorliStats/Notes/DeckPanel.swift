...
enum DeckStyle: String, CaseIterable {
    ...
    var title: String {
        switch self {
        case .tabs: return NotesL10n.text("deck_style.labelled_tabs")
        case .compact: return NotesL10n.text("deck_style.colour_chips")
        }
    }
}
...
enum DeckGeom {
    ...
    static var scale: CGFloat { CGFloat(NotesSettings.deckScale) }
    ...
    static var editorWidth: CGFloat { NotesSettings.noteSize.width }
    static var editorHeight: CGFloat { NotesSettings.noteSize.height }
...
final class DeckPanel: NSPanel {
    ...
    func applyLevel() {
        level = NotesSettings.showOverFullScreen ? .statusBar : .floating
        ...
        if NotesSettings.showOverFullScreen { behavior.insert(.fullScreenAuxiliary) }
...
final class DeckContentView: NSView {
    ...
    override func mouseDown(with event: NSEvent) {
        // Keep ordinary clicks available to the edge deck; Option-drag moves
        // the resting pill and preserves its exact vertical position.
