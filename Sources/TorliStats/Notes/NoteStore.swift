...
final class NoteStore: ObservableObject {
    ...
    private init() {
        notes = store.load()
        removeImportedWelcomeNoteIfNeeded()
        migrateLegacyTorliStickyNotesIfNeeded()
    ...
    /// themselves.
    /// Imports the temporary JSON notes used by Torli Stats before the complete
    /// Torli Notes engine was embedded. The original file stays untouched as a backup.
    private func removeImportedWelcomeNoteIfNeeded() {
        let historicProductName = "Not" + "y"
        let welcomeMarkers = [
            "Welcome to \(historicProductName)",
            "欢迎使用 \(historicProductName)",
            "welcome.note_body"
        ]
        let defaultNotes = notes.filter { note in
            welcomeMarkers.contains { note.body.hasPrefix($0) }
        }
    ...
    private func migrateLegacyTorliStickyNotesIfNeeded() {
        guard notes.isEmpty else { return }
        let torliSupport = Paths.support.deletingLastPathComponent()
        let legacyJSON = torliSupport.appendingPathComponent("sticky-notes.json", isDirectory: false)
        if let data = try? Data(contentsOf: legacyJSON),
           let legacy = try? JSONDecoder().decode([LegacyStickyNote].self, from: data) {
            for item in legacy where !item.isArchived && (!item.title.isEmpty || !item.body.isEmpty) {
                let color = StickyColorMigration.colorIndex(for: item.color)
                _ = create(body: item.body, title: item.title, color: color)
            }
            return
        }

        // The first Torli preview used one plain-text note before it gained the
        // JSON deck format. Preserve that text as well.
        let legacyTextURL = torliSupport.appendingPathComponent("sticky-note.txt", isDirectory: false)
        if let body = try? String(contentsOf: legacyTextURL, encoding: .utf8), !body.isEmpty {
            _ = create(body: body, color: 0)
        }
    }
...
private struct LegacyStickyNote: Decodable {
    let title: String
    let body: String
    let color: String
    let isArchived: Bool
...
private enum StickyColorMigration {
    static func colorIndex(for legacyColor: String) -> Int {
        switch legacyColor {
        case "amber": return 0
        case "coral": return 1
        case "rose": return 2
