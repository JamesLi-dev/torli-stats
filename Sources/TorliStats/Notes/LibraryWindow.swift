...
enum LibraryMode: String, CaseIterable, Identifiable {
    ...
    var title: String {
        switch self {
        case .all: NotesL10n.text("library.all_notes")
        case .archive: NotesL10n.text("library.archive")
        }
    }
}
...
final class LibraryWindow: NSObject, NSWindowDelegate {
    ...
    func windowWillClose(_ notification: Notification) {
        ...
        // unless NotesSettings is still up.
...
struct LibraryView: View {
    ...
    private var sidebar: some View {
        VStack(spacing: 0) {
            ...
            HStack(spacing: 6) {
                ...
                TextField(NotesL10n.text("library.search_placeholder"), text: $model.query)
            ...
            if filtered.isEmpty {
                VStack(spacing: 6) {
                    ...
                    Text(model.query.isEmpty
                         ? (model.mode == .all
                            ? NotesL10n.text("library.no_notes")
                            : NotesL10n.text("library.no_archive"))
                         : NotesL10n.text("library.no_matches"))
            ...
            HStack(spacing: 12) {
                Button { NoteStore.shared.create() } label: {
                    Label(NotesL10n.text("menu.new_note"), systemImage: "plus").font(.system(size: 11.5))
                }
                .buttonStyle(.plain).foregroundStyle(.secondary)

                Menu {
                    Button(NotesL10n.text("export.markdown_per_note")) {
                        Transfer.export(.markdown, notes: NoteStore.shared.notes)
                    }
                    Button(NotesL10n.text("export.plain_per_note")) {
                        Transfer.export(.plainText, notes: NoteStore.shared.notes)
                    }
                    Button(NotesL10n.text("export.single_document")) {
                        Transfer.export(.singleFile, notes: NoteStore.shared.notes)
                    }
                    Button(NotesL10n.text("export.sticky_archive")) {
                        Transfer.export(.stickies, notes: NoteStore.shared.notes)
                    }
                    Divider()
                    Button(NotesL10n.text("menu.import")) { Transfer.importFiles() }
                } label: {
                    Label(NotesL10n.text("menu.export"), systemImage: "square.and.arrow.up").font(.system(size: 11.5))
                }
    ...
    private func row(_ note: Note) -> some View {
        ...
        .contextMenu {
            if note.archived {
                Button(NotesL10n.text("action.restore")) { NoteStore.shared.setArchived(id: note.id, false) }
            } else {
    ...
    private var detail: some View {
        if let note = selected {
            ...
            VStack(spacing: 8) {
                ...
                Text(NotesL10n.text("library.select_note")).font(.system(size: 13)).foregroundStyle(.secondary)
            }
            .frame(maxWidth: .infinity, maxHeight: .infinity)
    ...
    var body: some View {
        VStack(spacing: 0) {
            HStack(spacing: 10) {
                ...
                .buttonStyle(.plain)
                .help(NotesL10n.text("help.pick_colour"))
                .contextMenu {
                ...
                Spacer()
                Text(NotesL10n.format("note.edited", Fmt.ago(note.modified)))
            ...
            NoteTextView(text: $text, ink: NSColor(pal.ink), bridge: bridge,
                         autofocus: false, fontSize: NotesSettings.noteFontSize,
                         markdownEnabled: NotesSettings.markdownStyling,
                         textDirection: note.textDirection,
                         styleToken: "\(note.color)|\(NotesSettings.noteFontSize)|\(NotesSettings.noteFontName)|\(NotesSettings.markdownStyling)")
