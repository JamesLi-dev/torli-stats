...
enum Transfer {
    ...
    static func export(_ format: Format, notes: [Note]) {
        guard !notes.isEmpty else {
            alert(NotesL10n.text("export.empty_title"), NotesL10n.text("export.empty_body"))
            return
        }
        NSApp.activate()
        switch format {
    ...
    private static func exportPerFile(_ notes: [Note], ext: String, render: (Note) -> String) {
        ...
        panel.prompt = NotesL10n.text("export.here")
        panel.message = NotesL10n.plural("export.choose_folder", notes.count, ext.uppercased())
        ...
        for n in notes {
            ...
            do {
                ...
                NSLog("Torli Stats notes export failed for \(url.lastPathComponent): \(error.localizedDescription)")
            }
        }
        reveal(dir)
        if written < notes.count {
            alert(NotesL10n.text("export.incomplete_title"),
                  NotesL10n.format("export.incomplete_body", written,
                              NotesL10n.plural("notes.count", notes.count)))
        }
    }

    private static func exportSingle(_ notes: [Note]) {
        let panel = NSSavePanel()
        panel.nameFieldStringValue = NotesL10n.format(
        ...
        let doc = notes.map { n -> String in
            let archiveSuffix = n.archived ? NotesL10n.text("export.metadata_archived_suffix") : ""
            let metadata = NotesL10n.format(
        ...
        let header = "# \(NotesL10n.text("export.document_title"))\n\n"
            + "\(NotesL10n.plural("notes.count", notes.count)) · \(Fmt.stamp.string(from: Date()))\n\n---\n\n"
    ...
    private static func exportArchive(_ notes: [Note]) {
        ...
        do {
            ...
            alert(NotesL10n.text("export.failed_title"), error.localizedDescription)
        }
    }
    ...
    static func importFiles() {
        ...
        panel.allowsOtherFileTypes = true
        panel.message = NotesL10n.text("import.choose_files")
        ...
        if failed.isEmpty {
            alert(NotesL10n.text("import.complete_title"), NotesL10n.plural("import.added", added))
        } else {
            alert(NotesL10n.text("import.problems_title"),
                  NotesL10n.format("import.problems_body", NotesL10n.plural("import.added", added),
    ...
    private static func safeName(_ n: Note) -> String {
        ...
        return trimmed.isEmpty
            ? NotesL10n.format("export.default_note_name", String(n.id.prefix(8)))
            : trimmed
    }
