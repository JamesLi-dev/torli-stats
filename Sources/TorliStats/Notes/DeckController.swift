...
final class DeckModel: ObservableObject {
    ...
    // Mirrored from NotesSettings so SwiftUI re-renders when a preference flips.
    @Published var style: DeckStyle = NotesSettings.deckStyle
    @Published var alwaysShown: Bool = NotesSettings.deckAlwaysShown
    @Published var pillHidden: Bool = NotesSettings.deckPillHidden
    /// DeckGeom reads the scale straight from NotesSettings; this mirror exists purely
    /// so a change to it invalidates the views that measure against it.
    @Published var scale: Double = NotesSettings.deckScale
    @Published var onLeftEdge: Bool = NotesSettings.deckOnLeftEdge
    @Published var fontSize: Double = NotesSettings.noteFontSize
    @Published var markdown: Bool = NotesSettings.markdownStyling
    @Published var noteSize = NotesSettings.noteSize
    @Published var openOnHover: Bool = NotesSettings.openOnHover
    @Published var tabPreview: Bool = NotesSettings.tabPreview
    ...
    @Published var openedHeight: CGFloat = NotesSettings.noteSize.height
...
final class DeckController: NSObject {
    ...
    private var restingState: DeckState { NotesSettings.deckAlwaysShown ? .fan : .rest }
    ...
    func layout(for state: DeckState) {
        ...
        let onRight = !NotesSettings.deckOnLeftEdge
        ...
        case .rest:
            ...
            let w = max(DeckGeom.pillWidth + 2, CGFloat(NotesSettings.edgeWidth))
            let availableH = max(1, vis.height - h)
            let y = vis.minY + availableH * NotesSettings.deckYRatio
    ...
    private func startIdleWatch() {
        ...
        idleTimer = Timer.scheduledTimer(withTimeInterval: 0.12, repeats: true) { [weak self] _ in
            ...
            case .fan where idle > NotesSettings.fanIdleTimeout:
                self.collapse()
            case .expanded where idle > NotesSettings.noteIdleTimeout && !self.openNoteIsPinned:
                self.dismiss()
            default: break
            }
        }
    ...
    /// Option-drag the pill between displays and snap it to the nearest edge
    /// while preserving the selected vertical position.
    func beginPillDrag(with initialEvent: NSEvent) {
        ...
        var targetOnLeft = NotesSettings.deckOnLeftEdge
        var targetYRatio = NotesSettings.deckYRatio
        ...
        NotesSettings.deckOnLeftEdge = targetOnLeft
        NotesSettings.deckYRatio = targetYRatio
    ...
    private var hotZone: NSRect {
        ...
        return NotesSettings.deckOnLeftEdge
    ...
    func expand(_ id: String) {
        ...
        if FloatingNote.shared.noteID == id {
            NSLog("Torli Notes: expand \(id.prefix(6)) routed to floating note")
            FloatingNote.shared.focus(); return
        }
        model.openedHeight = NotesSettings.noteSize.height
        model.openedTop = nil
        manager?.deckDidActivate(self)
    ...
    func detachExpandedNote(at pointer: NSPoint) {
        ...
        let size = NotesSettings.floatingNoteSize
    ...
    /// the pointer is off the deck. Called after NotesSettings writes it.
    ...
    private func installKeyMonitor() {
        ...
        keyMonitor = NSEvent.addLocalMonitorForEvents(matching: .keyDown) { [weak self] event in
            ...
            if NotesSettings.scClose.matches(event) {
                if self.model.findQuery != nil { self.model.findQuery = nil }
                else { self.collapse() }
                return nil
            }
            if NotesSettings.scArchiveNote.matches(event) {
                NoteStore.shared.setArchived(id: id, true); self.collapse(); return nil
            }
            if NotesSettings.scDelete.matches(event) {
                NoteStore.shared.delete(id: id); self.collapse(); return nil
            }
            if NotesSettings.scFind.matches(event) {
                self.model.findQuery = self.model.findQuery == nil ? "" : nil; return nil
            }
            if NotesSettings.scTask.matches(event) {
                self.model.bridge.toggleTaskLine(); return nil
            }
            if NotesSettings.scPin.matches(event) {
                NoteStore.shared.togglePin(id: id); return nil
            }
            if NotesSettings.scColour.matches(event) {
                NoteStore.shared.cycleColor(id: id); return nil
            }
            if NotesSettings.scBigger.matches(event) {
                NotesAppBridge.shared.delegate?.stepFontSize(by: 1.5); return nil
            }
            if NotesSettings.scSmaller.matches(event) {
                NotesAppBridge.shared.delegate?.stepFontSize(by: -1.5); return nil
            }
            return event
        }
    ...
    func showContextMenu(at event: NSEvent) {
        let menu = NSMenu()
        menu.addItem(withTitle: NotesL10n.text("menu.new_note"), action: #selector(TorliAppDelegate.newNote), keyEquivalent: "")
        menu.addItem(withTitle: NotesL10n.text("menu.all_notes"), action: #selector(TorliAppDelegate.openAllNotes), keyEquivalent: "")
        menu.addItem(withTitle: NotesL10n.text("menu.archive"), action: #selector(TorliAppDelegate.openArchive), keyEquivalent: "")
        menu.addItem(.separator())

        let overFS = NSMenuItem(title: NotesL10n.text("menu.show_over_fullscreen"),
                                action: #selector(TorliAppDelegate.toggleOverFullScreen), keyEquivalent: "")
        overFS.state = NotesSettings.showOverFullScreen ? .on : .off
        menu.addItem(overFS)

        let styleItem = NSMenuItem(title: NotesL10n.text("menu.deck_style"), action: nil, keyEquivalent: "")
        let styleMenu = NSMenu()
        for s in DeckStyle.allCases {
            let it = NSMenuItem(title: s.title, action: #selector(TorliAppDelegate.setDeckStyle(_:)), keyEquivalent: "")
            it.representedObject = s.rawValue
            it.state = NotesSettings.deckStyle == s ? .on : .off
            styleMenu.addItem(it)
        }
        styleItem.submenu = styleMenu
        menu.addItem(styleItem)

        let fontItem = NSMenuItem(title: NotesL10n.text("menu.note_font"), action: nil, keyEquivalent: "")
        let fontMenu = NSMenu()
        for f in Ink.faces {
            let it = NSMenuItem(title: f.localizedName, action: #selector(TorliAppDelegate.setNoteFont(_:)),
                                keyEquivalent: "")
        ...
        let textItem = NSMenuItem(title: NotesL10n.text("menu.text_size"), action: nil, keyEquivalent: "")
        let textMenu = NSMenu()
        for entry in NotesSettings.fontSizes {
            let it = NSMenuItem(title: NotesL10n.text(entry.nameKey), action: #selector(TorliAppDelegate.setFontSize(_:)),
                                keyEquivalent: "")
            it.representedObject = entry.size
            it.state = abs(NotesSettings.noteFontSize - entry.size) < 0.01 ? .on : .off
            textMenu.addItem(it)
        }
        textItem.submenu = textMenu
        menu.addItem(textItem)

        let sizeItem = NSMenuItem(title: NotesL10n.text("menu.deck_size"), action: nil, keyEquivalent: "")
        let sizeMenu = NSMenu()
        for entry in NotesSettings.deckSizes {
            let it = NSMenuItem(title: NotesL10n.text(entry.nameKey), action: #selector(TorliAppDelegate.setDeckScale(_:)),
                                keyEquivalent: "")
            it.representedObject = entry.scale
            it.state = abs(NotesSettings.deckScale - entry.scale) < 0.01 ? .on : .off
            sizeMenu.addItem(it)
        }
        sizeItem.submenu = sizeMenu
        menu.addItem(sizeItem)

        let keepOpen = NSMenuItem(title: NotesL10n.text("menu.keep_deck_open"),
                                  action: #selector(TorliAppDelegate.toggleDeckAlwaysShown), keyEquivalent: "")
        keepOpen.state = NotesSettings.deckAlwaysShown ? .on : .off
        menu.addItem(keepOpen)

        let leftEdge = NSMenuItem(title: NotesL10n.text("menu.dock_left"),
                                  action: #selector(TorliAppDelegate.toggleDeckEdge), keyEquivalent: "")
        leftEdge.state = NotesSettings.deckOnLeftEdge ? .on : .off
        menu.addItem(leftEdge)

        if NSScreen.screens.count > 1 {
            let displayItem = NSMenuItem(title: NotesL10n.text("menu.display"), action: nil, keyEquivalent: "")
            let displayMenu = NSMenu()

            let allItem = NSMenuItem(title: NotesL10n.text("display.all"), action: #selector(TorliAppDelegate.setDisplayTarget(_:)), keyEquivalent: "")
            allItem.representedObject = "all"
            allItem.state = NotesSettings.displayTarget == "all" ? .on : .off
            displayMenu.addItem(allItem)

            let mainItem = NSMenuItem(title: NotesL10n.text("display.main"), action: #selector(TorliAppDelegate.setDisplayTarget(_:)), keyEquivalent: "")
            mainItem.representedObject = "main"
            mainItem.state = NotesSettings.displayTarget == "main" ? .on : .off
            ...
            for screen in NSScreen.screens {
                ...
                let it = NSMenuItem(title: title, action: #selector(TorliAppDelegate.setDisplayTarget(_:)), keyEquivalent: "")
                it.representedObject = "id:\(id)"
                it.state = NotesSettings.displayTarget == "id:\(id)" ? .on : .off
        ...
        let exportItem = NSMenuItem(title: NotesL10n.text("menu.export"), action: nil, keyEquivalent: "")
        let exportMenu = NSMenu()
        exportMenu.addItem(withTitle: NotesL10n.text("export.markdown_per_note"),
                           action: #selector(TorliAppDelegate.exportMarkdown), keyEquivalent: "")
        exportMenu.addItem(withTitle: NotesL10n.text("export.plain_per_note"),
                           action: #selector(TorliAppDelegate.exportPlainText), keyEquivalent: "")
        exportMenu.addItem(withTitle: NotesL10n.text("export.single_document"),
                           action: #selector(TorliAppDelegate.exportSingleFile), keyEquivalent: "")
        exportMenu.addItem(withTitle: NotesL10n.text("export.sticky_archive"),
                           action: #selector(TorliAppDelegate.exportStickies), keyEquivalent: "")
        exportItem.submenu = exportMenu
        menu.addItem(exportItem)
        menu.addItem(withTitle: NotesL10n.text("menu.import"), action: #selector(TorliAppDelegate.importStickies), keyEquivalent: "")
        menu.addItem(.separator())
        menu.addItem(withTitle: NotesL10n.text("menu.settings"), action: #selector(TorliAppDelegate.openNoteSettings), keyEquivalent: "")
        menu.addItem(withTitle: NotesL10n.text("menu.quit"), action: #selector(TorliAppDelegate.quit), keyEquivalent: "")

        for item in menu.items where item.action != nil {
            item.target = NotesAppBridge.shared.delegate
        }
...
final class DeckManager {
    ...
    private func targetDisplayIDs() -> Set<CGDirectDisplayID> {
        ...
        let target = NotesSettings.displayTarget
        if target == "all" {
