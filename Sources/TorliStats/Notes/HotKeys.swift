...
final class HotKeys {
    ...
    private let signature: OSType = 0x54524C49   // 'TRLI'
    ...
    func register(newNote: @escaping () -> Void,
        ...
        handlers = [
            (1, { NotesSettings.scNewNote }, newNote),
            (2, { NotesSettings.scAllNotes }, allNotes),
            (3, { NotesSettings.scArchive }, archive),
            (4, { NotesSettings.scCapture }, capture),
        ]
        reload()
    }

    /// Re-reads the stored shortcuts and rebinds. Called after a change in
    /// NotesSettings, so a new combo takes effect without relaunching.
    func reload() {
        unregisterHotKeys()
        installHandler()
        for h in handlers {
    ...
    private func add(id: UInt32, key: UInt32, mods: UInt32, action: @escaping () -> Void) {
        ...
        if status == noErr {
            ...
            NSLog("Torli Notes: hotkey \(id) unavailable (status \(status)) — another app may own it")
        }
    }
