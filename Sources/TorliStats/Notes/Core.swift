...
enum Paths {
    static let support: URL = {
        let base = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
            .appendingPathComponent("TorliStats", isDirectory: true)
...
struct NoteColor {
    ...
    var localizedName: String {
        NotesL10n.text("color.\(name.lowercased())")
    }
...
struct NoteFace {
    ...
    var localizedName: String {
        body.isEmpty ? NotesL10n.text("font.system") : name
    }
}
...
enum Ink {
    ...
    static var face: NoteFace {
        let want = NotesSettings.noteFontName
...
enum NoteTextDirection: String, Codable, CaseIterable, Identifiable {
    ...
    var title: String {
        switch self {
        case .automatic:   NotesL10n.text("direction.automatic")
        case .leftToRight: NotesL10n.text("direction.left_to_right")
        case .rightToLeft: NotesL10n.text("direction.right_to_left")
        }
    }
...
struct Note: Identifiable, Hashable {
    ...
    var displayTitle: String {
        ...
        return derived.isEmpty ? NotesL10n.text("note.untitled") : derived
    }
...
enum Fmt {
    ...
    static func ago(_ d: Date) -> String {
        if Date().timeIntervalSince(d) < 60 { return NotesL10n.text("date.just_now") }
